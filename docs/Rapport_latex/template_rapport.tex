...
\begin{figure}[t]
	...
	\end{subfigure}
	\caption{Physical and simulated setups used to evaluate the proposed architecture. In both workflows, the robot-mounted camera observes static AprilTags distributed in the environment.}
...
As illustrated in block~\textbf{(2)} of Figure~\ref{fig:recording_pipeline}, frames containing multiple visible tags allow the construction of pairwise constraints. When two tags $i$ and $j$ are simultaneously detected, their relative transformation can be computed from the camera-centered estimates. Using the composition and inversion properties of rigid transformations \cite{Hartley2004,barfoot2017state}, the observed relation is given by Eq.~\eqref{eq:obs_rel_tagij}.
...
Each observation in Eq.~\eqref{eq:obs_rel_tagij} induces a constraint between the unknown world poses ${}^{\text{world}}\!T_{\text{tag}_i}$ and ${}^{\text{world}}\!T_{\text{tag}_j}$. In an ideal noise-free setting, the observed relative pose would match the relative pose implied by the world estimates, as expressed in Eq.~\eqref{eq:pred_rel_tagij}.
...
\end{equation}
The matrix $R_{err,ij}$ is then mapped to a minimal three-parameter representation $r_{R,ij}\in\mathbb{R}^{3}$ using an axis--angle (rotation vector) parametrization, which is consistent with the pose representation adopted for optimization \cite{barfoot2017state}. The complete residual used in Eq.~\eqref{eq:tagmap_nlsq} is formed by stacking the translational and rotational components, $\,\mathrm{err}_{ij} = [\,r_{t,ij}^{\top}\;\; r_{R,ij}^{\top}\,]^{\top}$.
...
Finally, as indicated in block~\textbf{(4)} of Figure~\ref{fig:recording_pipeline}, the optimized tag map is used to reconstruct the camera trajectory. For each observation of a tag $i$ at time $t$, a world-referenced camera pose estimate is obtained according to Eq.~\eqref{eq:world_cam_from_mapi}.
...
\begin{figure}[h]
	...
	\resizebox{0.95\textwidth}{!}{
		\begin{tikzpicture}[
			...
			{\textbf{(1)} Optional alignment\\(XY + yaw)};
	...
	\caption{Architecture of the relocalization pipeline with separated offline analysis and online execution. Step~(4*) loads the reference trajectory generated in the recording phase. Both workflows compute the distance to the reference trajectory $D$ and the heading deviation $\phi$ as control-oriented outputs.}
