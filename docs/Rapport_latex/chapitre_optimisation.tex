...
Given a known map, the camera's pose in the world frame ($T_{W,C}$) is estimated from a detected tag, \cite{Hartley2004}:
\begin{equation}
...
For each frame, LM solves the same equation used previously to estimate the graphs, \cite{barfoot2024state}:
