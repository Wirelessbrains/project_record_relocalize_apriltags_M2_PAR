...
def fun_cam_only(x_cams, num_cams, t_pos_fixed, anchor_pos, anchor_rot):
    ...
    res_obs = (batch_cam_rot.inv().apply(batch_tag_pos - batch_cam_pos) - obs_measurements).ravel() * 20.0 # Increased weight
    res_anchor_pos = (cam_poses_pos[0] - anchor_pos) * 1000.0
    res_anchor_rot = (cam_poses_rot[0] - anchor_rot) * 1000.0

    # Loop closure residuals: penalize difference between first and last pose
    res_loop_pos = (cam_poses_pos[num_cams-1] - cam_poses_pos[0]).ravel() * 100.0 # Weight heavily
    # Rotation difference: R_last * R_first_inv -> as_rotvec
    res_loop_rot = (R.from_rotvec(cam_poses_rot[num_cams-1]) * R.from_rotvec(cam_poses_rot[0]).inv()).as_rotvec().ravel() * 100.0 # Weight heavily

    # Smoothness residuals: penalize large changes between consecutive poses
    # Weight of 10.0, can be tuned
    res_smooth_pos = (cam_poses_pos[1:] - cam_poses_pos[:-1]).ravel() * 10.0
    res_smooth_rot = (R.from_rotvec(cam_poses_rot[1:]) * R.from_rotvec(cam_poses_rot[:-1]).inv()).as_rotvec().ravel() * 10.0
    
    return np.concatenate([res_obs, res_anchor_pos, res_anchor_rot, res_loop_pos, res_loop_rot, res_smooth_pos, res_smooth_rot])
...
num_residuals = num_observations * 3 + 6 + 6 + num_smooth_residuals # +6 for anchor, +6 for loop closure, +num_smooth_residuals for smoothness
...
# Position component (cam_poses_pos[num_cams-1] - cam_poses_pos[0])
...
row_idx += 3
# Rotation component (cam_poses_rot[num_cams-1] - cam_poses_rot[0])
...
for i in range(num_frames - 1):
    # Position (cam_poses_pos[i+1] - cam_poses_pos[i])
    ...
    row_idx += 3
    # Rotation (cam_poses_rot[i+1] - cam_poses_rot[i])
...
if len(filtered_sparse_optimized_poses) > 1:
    ...
    tck_x = splrep(t_sparse, x_coords_sparse, s=0) # Larger s for more smoothing
    tck_y = splrep(t_sparse, y_coords_sparse, s=0) # Larger s for more smoothing
    tck_z = splrep(t_sparse, z_coords_sparse, s=0) # Larger s for more smoothing
...
ax.plot(final_pos_aligned_for_shape[:, 0], final_pos_aligned_for_shape[:, 1], 'g-', linewidth=2.5, label='Trajetória Otimizada')
ax.plot(interpolated_gt_for_plot[:, 0], interpolated_gt_for_plot[:, 1], 'y--', linewidth=1.5, label='Interp. dos Triângulos Otimizados (Suavizada)')

# Poses da Câmera Otimizadas (esparsas)
ax.scatter(final_pos_aligned_for_shape[::10, 0], final_pos_aligned_for_shape[::10, 1], marker='^', facecolors='none', edgecolors='blue', s=80, label='Poses Otimizadas (Esparso)')

# Poses Iniciais da Câmera (Com Ruído)
x0_cam_pos_aligned_for_plot = (R_shape_align @ x0_cam_pos.T).T + t_shape_align
ax.scatter(x0_cam_pos_aligned_for_plot[:, 0], x0_cam_pos_aligned_for_plot[:, 1], c='purple', marker='+', s=50, alpha=0.6, label='Poses Iniciais da Câmera (Com Ruído)')
