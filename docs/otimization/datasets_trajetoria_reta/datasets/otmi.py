...
try:
    filename = 'trajectory_reto_6_tags_noisy_dataset.yaml'
...
for i, frame in enumerate(frames):
    # GT Absoluto (apenas para comparação final)
    ...
    # Preparar Movimento Relativo (Constraints de Suavização)
    if i < num_frames - 1:
...
def fun_traj_smooth(x_traj, t_pos_fixed):
    """
    Função corrigida: Usa álgebra de quaternions para o erro rotacional.
    """
    ...
    # Erro = R_pred^-1 * R_next (Deveria ser identidade)
...
# ==========================================
print("\nRealizando Alinhamento Global baseando-se nas TAGS (Corrige erro sistemático)...")

# Carregar GT das Tags
gt_tags_file = 'trajectory_scenario.yaml'
gt_tags_map_align = {}
...
# Preparar pontos para Procrustes: (Estimado vs Real)
src_pts = [] # Estimado (Otimizado)
dst_pts = [] # Real (GT)
...
# Calcular alinhamento baseado na nuvem de tags
if len(src_pts) >= 3:
...
for i in range(num_observations):
    ...
    # A rotação da câmera também precisa ser alinhada ao mundo
...
if len(projected_poses) > 0:
    plt.scatter(projected_poses[:, 0], projected_poses[:, 1], c='green', marker='^', s=30, alpha=0.4, label='Projeção das Medições')
plt.plot(aligned_cam_pos[:, 0], aligned_cam_pos[:, 1], 'b-', linewidth=2.5, label='SLAM Otimizado & Alinhado')

# Tags
plt.scatter(est_tag_pos_aligned[:, 0], est_tag_pos_aligned[:, 1], c='red', marker='x', s=100, zorder=5, label='Tags Estimadas')
